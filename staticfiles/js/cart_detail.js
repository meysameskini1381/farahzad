...
function renderCart(cart) {
    ...
    cart.items.forEach(item => {
        ...
        const originalPrice = item.product_price;
        const finalPrice = item.final_price;
        const totalItemPrice = item.total_price;
...
<li class="cart-item" data-id="${item.id}">
    <div class="cart-media">
        <a href="#" style="position: relative;">
            ...
            ${hasDiscount ? `
            <div style="position: absolute; top: 5px; right: 5px; background: #ff6b6b; color: white; padding: 2px 8px; border-radius: 4px; font-size: 11px; font-weight: bold;">
    ...
    <div class="cart-info-group">
        <div class="cart-info">
            ...
            ${hasDiscount ? `
                <p>
                    <del style="color: #999; font-size: 12px;">${formatPrice(originalPrice)}</del>
                    <span style="color: #ff6b6b; font-weight: bold; margin-right: 5px;">${formatPrice(finalPrice)}</span>
                </p>
            ` : `
                <p>قیمت واحد - ${formatPrice(finalPrice)}</p>
            `}
        </div>
        ...
        <div class="cart-action-group">
            ...
            <h6>${formatPrice(totalItemPrice)}</h6>
        </div>
    </div>
</li>
        `);
    ...
    // نمایش تخفیف کل (اگه وجود داشت)
    if (cart.total_discount && cart.total_discount > 0) {
        totalPrice.innerHTML = `
            <del style="color: #999; font-size: 14px;">${formatPrice(cart.original_total)}</del><br>
            <span style="color: #ff6b6b; font-weight: bold;">${formatPrice(cart.total_price)}</span>
            <small style="display: block; color: #28a745; font-size: 12px;">
                (شما ${formatPrice(cart.total_discount)} صرفه‌جویی کردید)
            </small>
        `;
    } else {
